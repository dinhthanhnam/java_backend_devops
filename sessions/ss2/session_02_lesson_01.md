...
2. **`order-service` (Dịch vụ đặt món):** Được phát triển sau, sử dụng các tính năng mới của **Java 21** (như Virtual Threads để tối ưu hiệu năng).
...
* **Phương án 2: Sử dụng Máy ảo (Virtual Machine - VM)**
  * Để cô lập hoàn toàn môi trường, bạn tạo ra 2 máy ảo riêng biệt (VM 1 chạy JDK 17 để chạy `user-service`, VM 2 chạy JDK 21 để chạy `order-service`).
...
```text
  ...
  (Biệt thự đơn lập riêng móng)          (Căn hộ chung cư chung móng)
     ...
     │       (Java 17)       │              │       (Java 17)       │
     ├───────────────────────┤              ├───────────────────────┤
     │       Guest OS        │              │     order-service     │
     │    (Hệ điều hành 1)   │              │       (Java 21)       │
     ├───────────────────────┤              ├───────────────────────┤
     │       Móng riêng      │              │                       │
     └───────────────────────┘              │     Móng dùng chung   │
     ┌───────────────────────┐              │    (Shared Kernel /   │
     │     order-service     │              │        Host OS)       │
     │       (Java 21)       │              │                       │
     ├───────────────────────┤              │                       │
     │       Guest OS        │              │                       │
     │    (Hệ điều hành 2)   │              │                       │
...
* **Ứng dụng:** Bạn có thể thiết kế nhà thứ nhất (`user-service`) theo phong cách cổ điển (JDK 17) và nhà thứ hai (`order-service`) theo phong cách hiện đại (JDK 21) mà không sợ ảnh hưởng đến nhau.
...
* **Ứng dụng:** Bên trong căn hộ của `user-service`, bạn trang trí nội thất kiểu cổ điển (chứa JDK 17), căn hộ của `order-service` trang trí kiểu hiện đại (chứa JDK 21). Môi trường runtime bên trong mỗi căn hộ hoàn toàn độc lập và không hề xung đột với nhau.
...
1. **Namespaces (Phân vùng cô lập):**
   ...
   * Ví dụ: Nhờ `NET Namespace`, container chứa `user-service` và container chứa `order-service` đều có thể tự do cấu hình lắng nghe tại cổng nội bộ `8080` mà không hề gây xung đột cổng mạng trên máy host.
...
Nếu bạn triển khai `user-service` (chạy JDK 17) và `order-service` (chạy JDK 21) trực tiếp trên hệ điều hành host (không dùng Docker và VM), điều gì sẽ xảy ra nếu lập trình viên thay đổi biến môi trường toàn cục `JAVA_HOME` trỏ về thư mục chứa JDK 21?
* *Gợi ý:* `order-service` sẽ hoạt động bình thường, nhưng `user-service` có thể gặp lỗi nếu lệnh khởi chạy của nó phụ thuộc vào biến `JAVA_HOME` toàn cục này để tìm thư viện chạy dịch vụ, dẫn đến sự bất ổn định hoặc crash tiến trình. Khi đóng gói bằng Docker, mỗi container mang sẵn một bản sao JDK độc lập bên trong, do đó sự thay đổi cấu hình môi trường bên ngoài máy host hoàn toàn không ảnh hưởng đến hoạt động của container.
